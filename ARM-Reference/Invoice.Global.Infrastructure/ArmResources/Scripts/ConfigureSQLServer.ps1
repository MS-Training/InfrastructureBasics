...
Configuration DeploySQLServer
{
    param (  
        ...
        [Parameter(Mandatory)]
        [string] $SysDataPath = "M:\MSSqlServer\MSSQL\DATA",
        [Parameter(Mandatory)]
        [string] $LogPath = "O:\MSSqlServer\MSSQL\DATA",
        [Parameter(Mandatory)]
        [string] $TempDBPath = "D:\MSSqlServer\MSSQL\DATA",
        ...
        [Boolean] $setSQLServiceAccount = $true,
        [Parameter(Mandatory)]
    ...
    Node localhost
    {
        ...
        $SysDataPath = $SysDataPath.replace('MSSqlServer', $InstanceFullName)
        ...
        Script ConfigureEventLog {
            ...
            SetScript  = {
                try {
                    ...
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig Event Log Source was Created"
            ...
            TestScript = {
                try {
                    ...
                    if ($pass) { Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig-EnableEventLogging $pass" }
            ...
            SetScript  = {
                try {
                    ...
                    if ($diskArray -eq $nothing) {
                        ...
                        Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig-H Drive not found"
                        throw "Drives not available as expected"

                    }
                    else { Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig-Drives are Ready" }
            ...
            SetScript  = {
                ...
                try { 
                    ...
                    if ($(test-path -path $root) -eq $true) {
                        ...
                        Add-NTFSAccess -Path $Root -Account "CREATOR OWNER" -AccessRights "FullControl" -InheritanceFlags ContainerInherit, ObjectInherit -PropagationFlags None -AccessType Allow
                        Add-NTFSAccess -Path $Root -Account "NT AUTHORITY\SYSTEM" -AccessRights "FullControl" -InheritanceFlags ContainerInherit, ObjectInherit -PropagationFlags None -AccessType Allow
                        Add-NTFSAccess -Path $Root -Account "BUILTIN\Administrators" -AccessRights "FullControl" -InheritanceFlags ContainerInherit, ObjectInherit -PropagationFlags None -AccessType Allow
                        Add-NTFSAccess -Path $Root -Account "BUILTIN\Users" -AccessRights "ReadAndExecute" -InheritanceFlags ContainerInherit, ObjectInherit -PropagationFlags None -AccessType Allow
                    }                         
                    ...
                    if ([string]::IsNullOrEmpty($errorMessage) -ne $true) {
                        Write-EventLog -LogName Application -source AzureArmTemplates -eventID 3001 -entrytype Error -message "SQLConfig-ConfigureStartUpPath $Root check Failed: $errorMessage"
                    }
                }
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "NTFS Permissions have been set for $Root"
            }           
            TestScript = { 
                ...
                if ($(test-path -path $root) -eq $true) {
                    if ((Get-NTFSEffectiveAccess -Path $Root -Account "NT AUTHORITY\SYSTEM" | Select-Object AccessRights).AccessRights -ne "FullControl") {

                        $pass = $false
                    }
                    if ((Get-NTFSEffectiveAccess -Path $Root -Account "BUILTIN\Administrators" | Select-Object AccessRights).AccessRights -ne "FullControl") {
                ...
                if ($Pass) {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig-ConfigureStartupPath-Test $pass"
                }
                else {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1001 -entrytype Warning -message "SQLConfig-ConfigureStartupPath falied test $pass"
                }
        ...
        Script ConfigureDataPath {
            ...
            SetScript  = {
                ...
                try { 
                    ...
                    if ($(test-path -path $root) -eq $true) {                       
                        ...
                        Add-NTFSAccess -Path $Root -Account "NT AUTHORITY\SYSTEM" -AccessRights "FullControl" -InheritanceFlags ContainerInherit, ObjectInherit -PropagationFlags None -AccessType Allow
                        Add-NTFSAccess -Path $Root -Account "BUILTIN\Administrators" -AccessRights "FullControl" -InheritanceFlags ContainerInherit, ObjectInherit -PropagationFlags None -AccessType Allow
                        Add-NTFSAccess -Path $Root -Account "NT SERVICE\MSSQLSERVER" -AccessRights "FullControl" -InheritanceFlags ContainerInherit, ObjectInherit -PropagationFlags None -AccessType Allow
                        Add-NTFSAccess -Path $Root -Account "NT SERVICE\SQLSERVERAGENT" -AccessRights "FullControl" -InheritanceFlags ContainerInherit, ObjectInherit -PropagationFlags None -AccessType Allow
                    }                         
                    ...
                    [string]$errorMessage = $Error[0].Exception
                    if ([string]::IsNullOrEmpty($errorMessage) -ne $true) {
                        Write-EventLog -LogName Application -source AzureArmTemplates -eventID 3001 -entrytype Error -message "SQLConfig-ConfigureDataPath: $errorMessage"
                    }
                }
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "NTFS Permissions have been set for $Root"
            }           
            TestScript = { 
                ...
                if ($(test-path -path $root) -eq $true) {
                    ...
                    if ((Get-NTFSEffectiveAccess -Path $Root -Account "NT SERVICE\MSSQLSERVER" | Select-Object AccessRights).AccessRights -ne "FullControl") {

                        $pass = $false
                    }
                    if ((Get-NTFSEffectiveAccess -Path $Root -Account "NT SERVICE\SQLSERVERAGENT" | Select-Object AccessRights).AccessRights -ne "FullControl") {
                ...
                if ($Pass) {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig-ConfigureDataPath-Test $pass"
                }
                else {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1001 -entrytype Warning -message "SQLConfig-ConfigureDataPath-Test Failed $pass"
                }
        ...
        File SQLSysDataPath {
            Type            = 'Directory'
            DestinationPath = $SysDataPath
            Ensure          = "Present"
            DependsOn       = "[Script]ConfigureDataPath"
        }
        Script ConfigureSysDataPath {
            GetScript  = {
                @{
                }
            }
            SetScript  = {
                ...
                try { 
                    ...
                    [string]$errorMessage = $Error[0].Exception
                    if ([string]::IsNullOrEmpty($errorMessage) -ne $true) {
                        Write-EventLog -LogName Application -source AzureArmTemplates -eventID 3001 -entrytype Error -message "SQLConfig-ConfigureSysDataPath: $errorMessage"
                    }
                }
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "NTFS Permissions have been set for $Root"
            }           
            TestScript = { 
                ...
                if ($Pass) {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig-ConfigureSysDataPath-Test $pass"
                }
                else {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1001 -entrytype Warning -message "SQLConfig-ConfigureSysDataPath-Test-Failed $pass"
                }

                return $pass
            }
            DependsOn  = "[File]SQLSysDataPath"
        }
        ...
        Script ConfigureLogPath {
            ...
            SetScript  = {
                ...
                try { 
                    ...
                    [string]$errorMessage = $Error[0].Exception
                    if ([string]::IsNullOrEmpty($errorMessage) -ne $true) {
                        Write-EventLog -LogName Application -source AzureArmTemplates -eventID 3001 -entrytype Error -message "SQLConfig-ConfigureLogPath: $errorMessage"
                    }
                }
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "NTFS Permissions have been set for $Root"
            }
            TestScript = { 
                ...
                if ($Pass) {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig-ConfigureLogPath-Test $pass"
                }
                else {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1001 -entrytype Warning -message "SQLConfig-ConfigureLogPath-Test-Failed $pass"
                }
        ...
        Script ConfigureTempdbPath {
            ...
            SetScript  = {
                ...
                try { 
                    ...
                    [string]$errorMessage = $Error[0].Exception
                    if ([string]::IsNullOrEmpty($errorMessage) -ne $true) {
                        Write-EventLog -LogName Application -source AzureArmTemplates -eventID 3001 -entrytype Error -message "SQLConfig-ConfigureTempdbPath: $errorMessage"
                    }
                }
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "NTFS Permissions have been set for $Root"
            }
            TestScript = { 
                ...
                if ($Pass) {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig-ConfigureTempdbPath-Test $pass"
                }
                else {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1001 -entrytype Warning -message "SQLConfig-ConfigureTempdbPath-Test-Failed $pass"
                }
        ...
        Script ConfigureErrorPath {
            ...
            SetScript  = {
                ...
                try { 
                    ...
                    [string]$errorMessage = $Error[0].Exception
                    if ([string]::IsNullOrEmpty($errorMessage) -ne $true) {
                        Write-EventLog -LogName Application -source AzureArmTemplates -eventID 3001 -entrytype Error -message "SQLConfig-ConfigureErrorPath: $errorMessage"
                    }
                }
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "NTFS Permissions have been set for $Root"
            }           
            TestScript = { 
                ...
                if ($Pass) {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig-ConfigureErrorPath-Test $pass"
                }
                else {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1001 -entrytype Warning -message "SQLConfig-ConfigureErrorPath-Test-Failed $pass"
                }
        ...
        Script MoveMasterFiles {
            ...
            SetScript  = {
                ...
                $ret = $false
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Starting MoveMasterFiles SQL State is $sqlstate"
                if ($sqlInstances -ne $null) {
                    if ($sqlInstances.State -ne 'Running') {
                        if ($sqlInstances.State -eq 'StartPending') {
                            Start-Sleep -Seconds 60
                        }
                        if ($sqlInstances.State -eq 'StopPending') {
                            Start-Sleep -Seconds 60
                        }
                    ...
                    try { 
                        ...
                        # Data.
                        ################################################################                     
                        $DataPath = $($using:SysdataPath)
                        $logPath = $($using:SysdataPath)
                        ...
                        [string]$errorMessage = $Error[0].Exception
                        if ([string]::IsNullOrEmpty($errorMessage) -ne $true) {
                            Write-EventLog -LogName Application -source AzureArmTemplates -eventID 3001 -entrytype Error -message "SQLConfig-MoveMasterFiles: $errorMessage"
                        }
                        else { $errorMessage }
                    }
                } 
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status              
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "MoveMasterFiles has Completed and SQL State is $sqlstate" 
            }
            TestScript = { 
                ...
                if ($pass) {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig-MoveMasterFiles-Test $pass"
                }
                else {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1001 -entrytype Warning -message "SQLConfig-MoveMasterFiles-Test-Failed $pass"
                }
                return $pass
            }
            DependsOn  = @("[Script]ConfigureDataPath", "[Script]ConfigureSysDataPath", "[Script]ConfigureLogPath", "[Script]ConfigureTempdbPath", "[Script]ConfigureErrorPath")
        }
        ...
        Script MoveModelFiles {
            ...
            SetScript  = {
                ...
                $ret = $false
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "MoveModelFiles has started and sql state is $sqlstate"
                ...
                if ($sqlInstances -ne $null) {
                    ...
                    try { 
                        ...
                        if ($(Test-Path -Path $dataPath -ErrorAction SilentlyContinue) -eq $true) {
                            ...
                            $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                            Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "MoveModelFilesSQLState Before File Move $sqlstate"
                            ...
                            $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                            Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "MoveModelFilesCompleted and SQL is $sqlstate"
                        }
                        ...
                        [string]$errorMessage = $Error[0].Exception
                        if ([string]::IsNullOrEmpty($errorMessage) -ne $true) {
                            Write-EventLog -LogName Application -source AzureArmTemplates -eventID 3001 -entrytype Error -message "SQLConfig-MoveModelFiles: $errorMessage"
                        }
                        else { $errorMessage }
                    }
                }
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "MoveModelFiles has Completed" 
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "MoveModelFiles SQL State $sqlstate"
            }
            TestScript = { 
                ...
                if ($Pass) {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig-MoveModelFiles-Test $pass"
                }
                else {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1001 -entrytype Warning -message "SQLConfig-MoveModelFiles-Test-Failed $pass"
                }

                return $pass
            }
            DependsOn  = @("[Script]MoveMasterFiles", "[Script]ConfigureDataPath", "[Script]ConfigureSysDataPath", "[Script]ConfigureLogPath", "[Script]ConfigureTempdbPath", "[Script]ConfigureErrorPath")
        }
        ...
        Script MoveMSDBFiles {
            ...
            SetScript  = {
                ...
                $ret = $false
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "MoveMSDBFiles has started and sql state is $sqlstate"
                ...
                if ($sqlInstances -ne $null) {
                    ...
                    try { 
                        ...
                        [string]$errorMessage = $Error[0].Exception
                        if ([string]::IsNullOrEmpty($errorMessage) -ne $true) {
                            Write-EventLog -LogName Application -source AzureArmTemplates -eventID 3001 -entrytype Error -message "SQLConfig-MoveMSDBFiles: $errorMessage"
                        }
                        else { $errorMessage }
                    }
                }
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Complete MoveMSDBFiles and SQL is $sqlstate"
            }
            TestScript = { 
                ...
                if ($Pass) {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig-MoveMSDBFiles-Test $pass"
                }
                else {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1001 -entrytype Warning -message "SQLConfig-MoveMSDBFiles-Test-Failed $pass"
                }

                return $pass
            }
            DependsOn  = @("[Script]MoveModelFiles", "[Script]ConfigureDataPath", "[Script]ConfigureSysDataPath", "[Script]ConfigureLogPath", "[Script]ConfigureTempdbPath", "[Script]ConfigureErrorPath")
        }
        ...
        Script MoveTempdbFiles {
            ...
            SetScript  = {
                ...
                $ret = $false
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Starting MoveTempdbFiles and SQL is $sqlstate"
                ...
                if ($sqlInstances -ne $null) {
                    ...
                    try { 
                        ...
                        #US:4722110 ,This Code is newly added as a new ndf file is coming for the Tempdb in SQL images
                        $q = "ALTER DATABASE [tempdb] MODIFY FILE (NAME = temp2, FILENAME = '$($TempPath)\tempdb_mssql_2.ndf')"
                        Invoke-Sqlcmd -Database master -Query $q  -QueryTimeout 10000 -ErrorAction SilentlyContinue
                ...
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Complete MoveTempdbFiles and SQL is $sqlstate"
            ...
            DependsOn  = @("[Script]MoveMSDBFiles", "[Script]ConfigureDataPath", "[Script]ConfigureSysDataPath", "[Script]ConfigureLogPath", "[Script]ConfigureTempdbPath", "[Script]ConfigureErrorPath")
        }
        ...
        Script ConfigureDefaultLocations {
            ...
            SetScript  = {
                ...
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "ConfigureDefaultLocations has started and sql state is $sqlstate"
                if ($sqlInstances -ne $null) {
                    if ($sqlInstances.State -ne 'Running') {
                        if ($sqlInstances.State -eq 'StartPending') {
                            Start-Sleep -Seconds 60
                    ...
                    try {
                        ...
                        [string]$errorMessage = $Error[0].Exception
                        if ([string]::IsNullOrEmpty($errorMessage) -ne $true) {
                            Write-EventLog -LogName Application -source AzureArmTemplates -eventID 3001 -entrytype Error -message "SQLConfig-ConfigureDefaultLocations: $errorMessage"
                        }
                    }
                }
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "ConfigureDefaultLocations has Completed"
            }
            TestScript = { 
                ...
                if ($Pass) {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig-ConfigureDefaultLocations-Test $pass"
                }
                else {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1001 -entrytype Warning -message "SQLConfig-ConfigureDefaultLocations-Test-Failed $pass"
                }

                return $pass
            }
            DependsOn  = @("[Script]MoveTempdbFiles", "[Script]ConfigureDataPath", "[Script]ConfigureSysDataPath", "[Script]ConfigureLogPath", "[Script]ConfigureTempdbPath", "[Script]ConfigureErrorPath")
        }
        ...
        Script AddTempdbFiles {
            ...
            SetScript  = {
                ...
                $ret = $false
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Starting AddTempdbFiles and SQL is $sqlstate"
                ...
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Complete AddTempdbFiles and SQL is $sqlstate"
            }
            TestScript = { 
            ...
            DependsOn  = @("[Script]ConfigureDefaultLocations", "[Script]ConfigureDataPath", "[Script]ConfigureSysDataPath", "[Script]ConfigureLogPath", "[Script]ConfigureTempdbPath", "[Script]ConfigureErrorPath")
        }
        ...
        Script ConfigureSQLServerService {
            ...
            SetScript  = {
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Starting ConfigureSQLServerService and SQL is $sqlstate"
                ...
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Complete ConfigureSQLServerService"
            }
            TestScript = { 
                $pass = $false
        ...
        Script ConfigureSQLAgentService {
            ...
            SetScript  = {
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Starting ConfigureSQLAgentService"
                ############################################             
                $null = [System.Reflection.Assembly]::LoadWithPartialName("Microsoft.SqlServer.ConnectionInfo") 
                $null = [System.Reflection.Assembly]::LoadWithPartialName("Microsoft.SqlServer.SMO")
                $null = [System.Reflection.Assembly]::LoadWithPartialName("Microsoft.SqlServer.SmoExtended")
                ...
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Complete ConfigureSQLAgentService"
            }
            TestScript = { 
                $pass = $false
        ...
        Script ConfigureServerLoginMode {
            ...
            SetScript  = {
                ...
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "ConfigureServerLoginMode has started and sql state is $sqlstate"
                if ($sqlInstances -ne $null) {                   
                    if ($sqlInstances.State -ne 'Running') {
                        if ($sqlInstances.State -eq 'StartPending') {
                            Start-Sleep -Seconds 60
                    ...
                    try {  
                        ...
                        [string]$errorMessage = $Error[0].Exception
                        if ([string]::IsNullOrEmpty($errorMessage) -ne $true) {
                            Write-EventLog -LogName Application -source AzureArmTemplates -eventID 3001 -entrytype Error -message "SQLConfig-ConfigureServerLoginMode: $errorMessage"
                        }
                    }
                }
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "ConfigureServerLoginMode has Completed"
            }
            TestScript = { 
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
                $pass = $false
                ...
                if ($Pass) {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig-ConfigureServerLoginMode-Test $pass"
                }
                else {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1001 -entrytype Warning -message "SQLConfig-ConfigureServerLoginMode-Test-Failed $pass"
                }
        ...
        Script ConfigureMaxDop {
            ...
            SetScript  = {
                ...
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "ConfigureMaxDop has started and sql state is $sqlstate"
                if ($sqlInstances -ne $null) {
                    if ($sqlInstances.State -ne 'Running') {
                        if ($sqlInstances.State -eq 'StartPending') {
                            Start-Sleep -Seconds 60
                    ...
                    try {
                        ...
                        [string]$errorMessage = $Error[0].Exception
                        if ([string]::IsNullOrEmpty($errorMessage) -ne $true) {
                            Write-EventLog -LogName Application -source AzureArmTemplates -eventID 3001 -entrytype Error -message "SQLConfig-ConfigureMaxDop: $errorMessage"
                        } 
                    }
                }
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "ConfigureMaxDop has Completed"
            }
            TestScript = { 
                ...
                if ($Pass) {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig-ConfigureMaxDop-Test $pass"
                }
                else {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1001 -entrytype Warning -message "SQLConfig-ConfigureMaxDop-Test-Failed $pass"
                }
                return $pass
            }
            DependsOn  = "[Script]AddTempdbFiles"
        ...
        Script ConfigureMaxMemory {
            ...
            SetScript  = {
                ...
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "ConfigureMaxMemory has started and sql state is $sqlstate"
                if ($sqlInstances -ne $null) {
                    if ($sqlInstances.State -ne 'Running') {
                        if ($sqlInstances.State -eq 'StartPending') {
                            Start-Sleep -Seconds 60
                    ...
                    try { 
                        ...
                        [string]$errorMessage = $Error[0].Exception
                        if ([string]::IsNullOrEmpty($errorMessage) -ne $true) {
                            Write-EventLog -LogName Application -source AzureArmTemplates -eventID 3001 -entrytype Error -message "SQLConfig-ConfigureMaxMemory: $errorMessage"
                        }
                    }
                }
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "ConfigureMaxMemory has Completed"
            }
            TestScript = { 
                ...
                if ($Pass) {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfi-ConfigureMaxMemory-Test $pass"
                }
                else {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1001 -entrytype Warning -message "SQLConfig-ConfigureMaxMemory-Test-Failed $pass"
                }
        ...
        Script ConfigureModelDataFile {
            ...
            SetScript  = {
                ...
                $ret = $false
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Starting ConfigureModelDataFile and SQL is $sqlstate"
                ...
                if ($sqlInstances -ne $null) {
                    ...
                    try { 
                        ...
                        Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Trying to ConfigureModelDataFile Completed"
                       
                    }
                    catch {
                        [string]$errorMessage = $Error[0].Exception
                        if ([string]::IsNullOrEmpty($errorMessage) -ne $true) {
                            Write-EventLog -LogName Application -source AzureArmTemplates -eventID 3001 -entrytype Error -message "SQLConfig-ConfigureModelDataFile in catch: $errorMessage"
                        }
                        Write-EventLog -LogName Application -source AzureArmTemplates -eventID 3001 -entrytype Error -message "SQLConfig-ConfigureModelDataFile in catch2: $errorMessage"
                    }
                }
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Complete ConfigureModelDataFile and SQL is $sqlstate"
            }
            TestScript = { 
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
                ...
                if ($Pass) {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig-ConfigureModelDataFile-Test $pass"
                }
                else {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1001 -entrytype Warning -message "SQLConfig-ConfigureModelDataFile-Test-Failed $pass"
                }
        ...
        Script ConfigureModelLogFile {
            ...
            SetScript  = {
                ...
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Starting ConfigureModelLogFile and SQL is $sqlstate"
                ...
                if ($sqlInstances -ne $null) {
                    ...
                    try { 
                        ...
                        [string]$errorMessage = $Error[0].Exception
                        if ([string]::IsNullOrEmpty($errorMessage) -ne $true) {
                            Write-EventLog -LogName Application -source AzureArmTemplates -eventID 3001 -entrytype Error -message "SQLConfig-ConfigureModelLogFile: $errorMessage"
                        }
                    }
                }
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Complete ConfigureModelLogFile and SQL is $sqlstate"
            }
            TestScript = { 
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
                ...
                if ($Pass) {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig-ConfigureModelLogFile-Test $pass"
                }
                else {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1001 -entrytype Warning -message "SQLConfig-ConfigureModelLogFile-Test=Failed $pass"
                }
        ...
        Script ConfigureMSDBDataFile {
            ...
            SetScript  = {
                ...
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Starting ConfigureMSDBDataFile and SQL is $sqlstate"
                ...
                if ($sqlInstances -ne $null) {                  
                    ...
                    try {
                        ...
                        [string]$errorMessage = $Error[0].Exception
                        if ([string]::IsNullOrEmpty($errorMessage) -ne $true) {
                            Write-EventLog -LogName Application -source AzureArmTemplates -eventID 3001 -entrytype Error -message "SQLConfig-ConfigureMSDBDataFile: $errorMessage"
                        }
                        else { $errorMessage }
                    }
                }
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Complete ConfigureMSDBDataFile and SQL is $sqlstate"
            }
            TestScript = { 
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
                ...
                if ($Pass) {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig-ConfigureMSDBDataFile-Test $pass"
                }
                else {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1001 -entrytype Warning -message "SQLConfig-ConfigureMSDBDataFile-Test-Failed $pass"
                }
        ...
        Script ConfigureMSDBLogFile {
            ...
            SetScript  = {
                ...
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Starting ConfigureMSDBLogFile and SQL is $sqlstate"
                ...
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Complete ConfigureMSDBLogFile and SQL is $sqlstate"
            }
            TestScript = { 
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
        ...
        Script ConfigureAuditing {
            ...
            SetScript  = {
                ...
                $ret = $false
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Starting ConfigureAuditing and SQL is $sqlstate"
                ...
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Complete ConfigureAuditing and SQL is $sqlstate"
            }
            TestScript = { 
        ...
        Script ConfigureBuiltInAdmins {
            ...
            SetScript  = {
                ...
                $ret = $false
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Starting ConfigureBuiltInAdmins and SQL is $sqlstate"
                ...
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Complete ConfigureBuiltInAdmins and SQL is $sqlstate"
            }
            TestScript = { 
                try {
                    $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
        ...
        Script ConfigureTempDataFile {
            ...
            SetScript  = {
                ...
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Starting ConfigureTempDataFile and SQL is $sqlstate"
                ...
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Complete ConfigureTempDataFile and SQL is $sqlstate"
            }
            TestScript = { 
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
        ...
        Script ConfigureTempLogFile {
            ...
            SetScript  = {
                ...
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Starting ConfigureTempLogFile and SQL is $sqlstate"
                ...
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Complete ConfigureTempLogFile and SQL is $sqlstate"
            }
            TestScript = { 
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
        ...
        Script ConfigureMasterDataFile {
            ...
            SetScript  = {
                ...
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Starting ConfigureMasterDataFile and SQL is $sqlstate"
                if ($sqlInstances -ne $null) {
                    if ($sqlInstances.State -ne 'Running') {
                        if ($sqlInstances.State -eq 'StartPending') {
                            Start-Sleep -Seconds 60
                ...
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Complete ConfigureMasterDataFile and SQL is $sqlstate"
            }
            TestScript = { 
        ...
        Script ConfigureMasterLogFile {
            ...
            SetScript  = {
                ...
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Starting ConfigureMasterLogFile and SQL is $sqlstate"
                if ($sqlInstances -ne $null) {
                    if ($sqlInstances.State -ne 'Running') {
                        if ($sqlInstances.State -eq 'StartPending') {
                            Start-Sleep -Seconds 60
                ...
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Complete ConfigureMasterLogFile and SQL is $sqlstate"
            }
            TestScript = { 
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
        ...
        Script ConfigureStartupJob {
            ...
            SetScript  = {
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Starting ConfigureStartupJob"
                if ($(test-path -path C:\SQLStartup) -eq $true) {
               
                    $WebClient = New-Object System.Net.WebClient
                    $WebClient.DownloadFile($($Using:scriptFolderUrl) + "SQL-Startup.ps1" + $($Using:sastoken), "C:\SQLStartup\SQL-Startup.ps1")

                    if ($(test-path -path C:\SQLStartup\SQL-Startup.ps1) -eq $true) {
                        C:\SQLStartup\SQL-Startup.ps1 $($using:TempDBPath)
                    }
                }
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Complete ConfigureStartupJob"
            }
            TestScript = { 
                $pass = $false
                if ($(test-path -path "C:\SQLStartup\SQL-Startup.ps1") -eq $true) {
        ...
        Script ConfigureExtendedSprocs {
            ...
            SetScript  = {
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Starting ConfigureExtendedSprocs"
                if ($(test-path -path C:\SQLStartup) -eq $true) {
               
                    $WebClient = New-Object System.Net.WebClient
                    $WebClient.DownloadFile($($Using:scriptFolderUrl) + "PostConfiguration.sql" + $($Using:sastoken), "C:\SQLStartup\PostConfiguration.sql")
                ...
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Complete ConfigureExtendedSprocs"
            }
            TestScript = { 
                $pass = $false
                if ($(test-path -path "C:\SQLStartup\PostConfiguration.sql") -eq $true) {
        ...
        Script ConfigureSQLAccount {
            ...
            SetScript  = {
                ...
                $ret = $false
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Starting ConfigureSQLAccount and SQL is $sqlstate"
                if ($sqlInstances -ne $null) {
                    if ($sqlInstances.State -ne 'Running') {
                        if ($sqlInstances.State -eq 'StartPending') {
                            Start-Sleep -Seconds 60
                ...
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Complete ConfigureSQLAccount and SQL is $sqlstate"
            }
            TestScript = { 
                try {
                    $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
        ...
        Script ConfigureLocalPolicy {
            ...
            SetScript  = {
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Starting ConfigureLocalPolicy"  
                #################Policy Changes####################################
                ...
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "Complete ConfigureLocalPolicy"
            }
            TestScript = { 
        ...
        Script ConfigureSQLAgent {
            ...
            SetScript  = {
                ...
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
                $sqlstate = (Get-Service -Name MSSQLSERVER).Status
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "ConfigureSQLAgent has started and sql state is $sqlstate"
                if ($sqlInstances -ne $null) {
                    if ($sqlInstances.State -ne 'Running') {
                        if ($sqlInstances.State -eq 'StartPending') {
                            Start-Sleep -Seconds 60
                    ...
                    try {   
                        ...
                        if ($sqlInstances.State -eq 'StartPending') {
                            "$(Get-Date -Format g) Waiting for SQL Server to start."
                        ...
                        [string]$errorMessage = $Error[0].Exception
                        if ([string]::IsNullOrEmpty($errorMessage) -ne $true) {
                            Write-EventLog -LogName Application -source AzureArmTemplates -eventID 3001 -entrytype Error -message "SQLConfig-ConfigureSQLAgent: $errorMessage"
                        }
                    }
                }
                Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1005 -entrytype Information -message "ConfigureSQLAgent has Completed"
            }
            TestScript = { 
                $sqlInstances = gwmi win32_service -computerName localhost -ErrorAction SilentlyContinue | ? { $_.Name -match "mssql*" -and $_.PathName -match "sqlservr.exe" } 
                ...
                if ($Pass) {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "SQLConfig-ConfigureSQLAgent-Test $pass"
                }
                else {
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1001 -entrytype Warning -message "SQLConfig-ConfigureSQLAgent-Test-Failed $pass"
                }

                return $pass
            }
            DependsOn  = "[Script]ConfigureLocalPolicy"
        }   
    }
