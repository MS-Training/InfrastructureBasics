            ...
            SetScript = {
                    ...
                    try { 
                        ...
                        if($(test-path -path $root) -eq $true) {
                            Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "Starting ConfigurePowerPatchPath"
                            $ACL = Get-Acl $Root
                       ...
                       [string]$errorMessage = $Error[0].Exception
                       if([string]::IsNullOrEmpty($errorMessage) -ne $true) {
                            Write-EventLog -LogName Application -source AzureArmTemplates -eventID 3001 -entrytype Error -message "ConfigurePowerPatchPath: $errorMessage"
                       }
                    }
                }           
            TestScript = { 
                ...
                if($(test-path -path $root) -eq $true) {
                    ...
                    if($($ACL | %{ $_.access | ?{$_.IdentityReference -eq 'BUILTIN\Users'}}).FileSystemRights -notmatch 'ReadAndExecute'){
                        $pass= $false
                    }                      
                ...
                if($Pass){
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1000 -entrytype Information -message "ConfigurePowerPatchPath $pass"
                }else{
                    Write-EventLog -LogName Application -source AzureArmTemplates -eventID 1001 -entrytype Warning -message "ConfigurePowerPatchPath Failed $pass"
                }
