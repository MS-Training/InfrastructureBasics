...
<#
.SYNOPSIS
    Copies modules and AzureRM MSI files need for configuring a hybrid worker.
.DESCRIPTION
    Copies modules and AzureRM MSI files need for configuring a hybrid worker.
.OUTPUTS
    None
...
function Main{
 try
	{
		# Load Settings File and Login
        Write-Verbose $("proxyAccount is: " + $proxyAccount) -Verbose		            
        Write-Verbose $("providing privilege: " + $privilege) -Verbose		            
            
        Write-Host " Starting running 'Grant privilege for the proxy account'"            
        #Invoke-Command -ArgumentList($proxyAccount,$privilege) -ScriptBlock{
		#param([string]$proxyAccount, [string] $privilege)
...
function Grant-UserRight {
 ...
 #>
    ...
    param (
        ...
        [Alias('System','ComputerName','Host')][String] $Computer = $env:COMPUTERNAME
    )
    process {
...
function Revoke-TokenPrivilege {
 ...
 #>
    ...
    #############
	Grant-UserRight -Account $proxyAccount -Right $privilege -Computer $env:COMPUTERNAME
#}

        Write-Host "Successfully completed 'Grant privilege for the proxy account'"
}
	catch
	{
		 Write-Verbose "Error running the localsecurityPolicyUpdate script" -Verbose
